"""
Amended version of
https://github.com/edx/edx-platform/blob/master/common/djangoapps/util/memcache.py
that takes the derex project name into account, so that more than one project
can be served by a single memcached process.
...
DEREX_PROJECT = os.environ.get("DEREX_PROJECT", "derex")
...
def safe_key(key, key_prefix, version):
    ...
    combined = ":".join([DEREX_PROJECT, key_prefix, version, key])
