...
class GitLogs(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if hasattr(settings, 'MONGODB_LOG'):
            for config_item in ['host', 'user', 'password', 'db', 'authsource']:
                mongo_db[config_item] = settings.MONGODB_LOG.get(
                    config_item, mongo_db[config_item])

        # Make sure special characters in user and password are quoted
        if mongo_db['user'] or mongodb['password']:
            mongo_db['user'] = quote_plus(mongo_db['user'])
            mongo_db['password'] = quote_plus(mongo_db['password'])

        mongouri = 'mongodb://{user}:{password}@{host}/{db}?authSource={authsource}'.format(**mongo_db)
